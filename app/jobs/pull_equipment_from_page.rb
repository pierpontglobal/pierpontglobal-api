  ...
  def perform(*args)
    ...
    vehicles.each do |vehicle|
      ...
      if !heavy_vehicle.present?
        ...
        equipment_type = HeavyVehicleType.where("TRIM(lower(name)) = ?", vehicle[:equipment_type].to_s.downcase.gsub(" ", ""))[0]
        ...
        manufacturer = HeavyVehicleManufacturer.where("TRIM(lower(name)) = ?", vehicle[:manufacturer].to_s.downcase.gsub(" ", ""))[0]
